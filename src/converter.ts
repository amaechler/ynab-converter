...
function convertDateToYNAB(date: string): string {
...
export function convertToYNAB(csvContent: string): string {
  const lines = csvContent.split('\n');
  const output: string[] = ['Date,Payee,Memo,Amount'];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const ynabTransaction: YNABTransaction = {
